	...
	if(isset($_GET['sendMail'])){
		...
		if($_GET['sendMail'] == 'queryForm'){
			...
			$MailBody=	'<div style="margin-left:5%">';
				extract($_GET);
				extract($_POST);
				$MailBody	=	'<span>Senders Name:</span> '.$senderName;
				$MailBody	=	'<span>Senders Email address:</span> '.$senderEmail;
				$MailBody	=	'<span>Query Subject:</span> '.$querySubject;
				$MailBody	=	'<span>Query Description:</span> '.$queryDescription;
			$MailBody=	'</div>';
