...
<head>
    ...
    <script>
		...
		function reOrderNews(){
			var elemsNews    =   $('[newsID]');
            var IdOrder =   '';
            var PriorityOrder   =   '';
            for(var i = 0; i< elemsNews.length; i++){
                if(IdOrder != "")
                    IdOrder+= ',';
                if(PriorityOrder != "")
                    PriorityOrder+= ',';
                
                IdOrder += $(elemsNews[i]).attr('newsID');
                PriorityOrder += i;
            }
		...
		function submitOrder(IdOrder, PriorityOrder){
			var url		=	'../../actions/savePriorityOrders.php?dataType=News&IdOrder='+IdOrder+'&PriorityOrder='+PriorityOrder;
			var	Method	=	'POST';
...
<body>
    ...
    <div style="float:left;width:100%; height:100px; margin:auto; text-align:center">
      <div style="width:90%; margin: 1% 5%; text-align:left; font-weight:700">
       In order to reorder the news, just drag and drop the news list below so as to reorder the news items. The one at the top has highest priority, and will be shown at the top in respective page to end users. Once the reordering is done press the save button.
       </div>
        <ul id="sortable" style="width:90%; margin: 2% 5%">
         <?php
            for($i= 0 ; $i< count($NewsData); $i++){
                $rowData	=	$NewsData[$i];
                ?>
                   <li newsID="<?php echo $rowData['ID']; ?>" class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span><?php echo urldecode($rowData['NewsTitle']); ?></li>
                <?php
            }
        ?>
        </ul>
        <button type="button" class="btn btn-primary" onclick="reOrderNews();">Save order</button>
    </div>
    <?php
		}
	?>
